...
In particular, for this model we developed the property `butter_at_table`
that checks that, eventually, the `butter` object is at the table (in position `(0, 0)`).
Copy the file `properties/properties_scan.xml` in the SCXML model directory generated by AS2FM,
together with all the other files.

```console
cp properties/properties_scan.xml scxml/
```
...
```console
scan [SCXML/MODEL/DIR/] verify butter_at_table
```
...
```console
scan [SCXML/MODEL/DIR/] trace butter_at_table --traces 100
```
