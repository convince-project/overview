...
// File autogenerated at 2026-04-22 10:05:31 CEST with M2C version 1.0.0
...
bool PlaceObjectSkill::start(int argc, char*argv[])
{
  ...
  m_actionClient = rclcpp_action::create_client<overarching_msgs::action::Place>(m_node, "/place_object");
  ...
  m_stateMachine.connectToEvent("place_object.Place.SendGoal", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "PlaceObjectSkill::place_object.Place.SendGoal");
    ...
    rclcpp_action::create_client<overarching_msgs::action::Place>(nodePlace, "/place_object");
    overarching_msgs::action::Place::Goal goal_msg;
    
    send_goal(goal_msg);
    RCLCPP_INFO(m_node->get_logger(), "done send goal");
  });
  m_stateMachine.connectToEvent("place_object.Place.ResultRequest", [this]([[maybe_unused]]const QScxmlEvent & event){
      RCLCPP_INFO(m_node->get_logger(), "PlaceObjectSkill::place_object.Place.ResultRequest");
      std::shared_ptr<rclcpp::Node> nodePlace = rclcpp::Node::make_shared(m_name + "SkillNodePlace");
      rclcpp_action::Client<overarching_msgs::action::Place>::SharedPtr clientPlace  =
        rclcpp_action::create_client<overarching_msgs::action::Place>(nodePlace, "/place_object");
      RCLCPP_INFO(m_node->get_logger(), "result request");
  });
  m_stateMachine.connectToEvent("place_object.Place.Feedback", [this]([[maybe_unused]]const QScxmlEvent & event){
      RCLCPP_INFO(m_node->get_logger(), "place_object.Place.Feedback");
      QVariantMap data;
      m_feedbackMutex.lock();
      
      m_feedbackMutex.unlock();
      m_stateMachine.submitEvent("place_object.Place.FeedbackReturn", data);
      RCLCPP_INFO(m_node->get_logger(), "place_object.Place.FeedbackReturn");
  });
...
void PlaceObjectSkill::send_goal(overarching_msgs::action::Place::Goal goal_msg)
{
  ...
  while (!m_actionClient->wait_for_action_server(std::chrono::seconds(1))) {
    ...
    if(retries == SERVICE_TIMEOUT) {
      ...
      m_stateMachine.submitEvent("place_object.Place.GoalResponse", data);
      break;
    }
  }
  if (wait_succeded) {
...
void PlaceObjectSkill::result_callback(const  rclcpp_action::ClientGoalHandle<overarching_msgs::action::Place>::WrappedResult & result)
{
  ...
  m_stateMachine.submitEvent("place_object.Place.ResultResponse", data);
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "place_object.Place.ResultResponse");
}
void PlaceObjectSkill::goal_response_callback(const rclcpp_action::ClientGoalHandle<overarching_msgs::action::Place>::SharedPtr & goal_handle)
{
  ...
  if (!goal_handle) {
    data.insert("call_succeeded", false);
    m_stateMachine.submitEvent("place_object.Place.GoalResponse", data);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "place_object.Place.GoalResponse Failure");
    RCLCPP_ERROR(m_node->get_logger(), "Goal was rejected by server");
  } else {
    data.insert("call_succeeded", true);
    m_stateMachine.submitEvent("place_object.Place.GoalResponse", data);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "place_object.Place.GoalResponse Success");
