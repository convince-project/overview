...
export ROS_WS=/convince_ws

# Convenience functions
function convince_ws() {
        cd ${ROS_WS}
}
function convince_src() {
        cd ${ROS_WS}/src
}
...
function convince_build_packages() {
        cwd=$(pwd)
        convince_ws
...
function convince_build_packages_up_to() {
        cwd=$(pwd)
        convince_ws
...
function convince_clean () {
        cwd=$(pwd)
        convince_ws
        local _ret=$?
        if [ $_ret -ne 0 ] ; then
                echo "Could not switch dirs. Aborting convince_clean"
                return $_ret
        fi
        rm -r build/*
        rm -r install/*
