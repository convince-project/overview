...
class ROSMonitor_place_monitor(Node):


	def callbackrobot_exec_action_name(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='robot/exec_action_name'
	...
	def callbackrobot_force_effort_z(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='robot/force_effort_z'
		dict['time']=float(self.get_clock().now().to_msg().sec) + float((self.get_clock().now().to_msg().nanosec) / 1000000000)
		self.ws_lock.acquire()
		while dict['time'] in self.dict_msgs:
			dict['time']+=0.01
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['robot/exec_action_name']={'package': 'std_msgs.msg', 'type': 'String', 'qos_reliability': 'reliable'}
		self.topics_info['robot/force_effort_z']={'package': 'std_msgs.msg', 'type': 'Float32', 'qos_reliability': 'reliable'}
		self.config_subscribers['robot/exec_action_name']=self.create_subscription(topic='robot/exec_action_name',msg_type=String,callback=self.callbackrobot_exec_action_name,qos_profile=QoSProfile(reliability=ReliabilityPolicy.RELIABLE, durability=DurabilityPolicy.VOLATILE, depth=1000))

		self.config_subscribers['robot/force_effort_z']=self.create_subscription(topic='robot/force_effort_z',msg_type=Float32,callback=self.callbackrobot_force_effort_z,qos_profile=QoSProfile(reliability=ReliabilityPolicy.RELIABLE, durability=DurabilityPolicy.VOLATILE, depth=1000))
...
def main(args=None):
	...
	log = './log.txt'
	actions = {}
	actions['robot/exec_action_name']=('log',0)
	actions['robot/force_effort_z']=('log',0)
	monitor = ROSMonitor_place_monitor('place_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
